...
using static Bamboomaui.Crypto.MLSAGNative;

namespace Bamboomaui.Crypto
{
    public record Generator(byte[] KeyImage, byte[] Pc, byte[] Ps);

    public class Mlsag : IDisposable
    {
        ...
        public byte[] ToKeyImage(byte[] sk, byte[] pk)
        {
            ...
            byte[] keyImage = null;
            if (pk.Length != Constant.PUBLIC_KEY_COMPRESSED_SIZE) return null;
        ...
        /// <returns></returns>
        public unsafe bool Prepare(Span<byte> publicKeys, Span<byte> blindSumOut, int nOuts, int nBlinded, int nCols, int nRows, Span<byte[]> inputs, Span<byte[]> outputs, Span<byte[]> blinds)
        {
        ...
        /// 
        /// </summary>
        /// <param name="kiOut"></param>
        /// <param name="pcOut"></param>
        /// <param name="psOut"></param>
        ...
        /// <param name="publicKeys"></param>
        /// <returns></returns>
        public unsafe bool Generate(Span<byte> kiOut, Span<byte> pcOut, Span<byte> psOut, Span<byte> nonce, Span<byte> preimage, int nCols, int nRows, int index, Span<byte[]> blinds, Span<byte> publicKeys)
        {
        ...
        /// <param name="ps"></param>
        /// <returns></returns>
        public unsafe bool Verify(Span<byte> preimage, int nCols, int nRows, Span<byte> publicKeys, Span<byte> ki, Span<byte> pc, Span<byte> ps)
        {

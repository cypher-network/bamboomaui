...
namespace Bamboomaui.Crypto
{
    public unsafe class Util
    {
        ...
        /// <returns></returns>
        public static IntPtr[] ToPublicKeysPtrs(Span<byte> publicKeys, int length)
        {
            var ptrPubKeys = new IntPtr[length];
            for (var i = 0; i < length; i++)
            {
                var ptr = Marshal.AllocHGlobal(33);
                Marshal.Copy(publicKeys.Slice(i * 33, 33).ToArray(), 0, ptr, 33);
                ptrPubKeys[i] = ptr;
            }
